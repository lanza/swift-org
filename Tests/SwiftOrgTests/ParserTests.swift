...
private func eval(_ node: Node?, makesure: (Section) -> Void) {
...
class ParserTests: XCTestCase, @unchecked Sendable {
  ...
  func testPlanning() {
    ...
    eval(doc.content[0]) { sec in
      ...
      XCTAssertEqual(
        planning.timestamp?.date, quickDate(date: date, time: time))
    }
  ...
  func testParseBlock() {
    ...
    XCTAssertEqual(
      block1.content,
      ["  class HelloWorld {", "  # print(\"Hell World\");", "  }"])
    ...
    XCTAssertEqual(
      block3.params!,
      ["yaml", "exports:", "results", ":results", "value", "html"])
  ...
  func testParseTable() {
    ...
    XCTAssertEqual(
      table.rows[1].cells, ["Bruce Wayne", "Human", "M", "Batman"])
    XCTAssertFalse(table.rows[1].hasSeparator)
    XCTAssertEqual(
      table.rows[2].cells, ["Clark Kent", "Kryptonian", "M", "Superman"])
    XCTAssertFalse(table.rows[2].hasSeparator)
    XCTAssertEqual(
      table.rows[3].cells, ["Diana Prince", "Amazonian", "F", "Wonder Woman"])
    XCTAssertFalse(table.rows[2].hasSeparator)
  }
