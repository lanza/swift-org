...
class TokenizerTests: XCTestCase, @unchecked Sendable {
  ...
  func testTokenHeading() {
    ...
    evalHeadline(
      "* TODO Level One with todo", stars: 1, text: "TODO Level One with todo")
  ...
  func testTokenPlanning() {
    ...
    evalPlanning(
      "CLOSED: [\(date) \(day) \(time)]",
      keyword: "CLOSED",
      timestamp: Timestamp(active: false, date: theDate, repeater: nil))
    evalPlanning(
      "SCHEDULED: <\(date) \(day) \(time) +2w>",  // with repeater
      keyword: "SCHEDULED",
      timestamp: Timestamp(active: true, date: theDate, repeater: "+2w"))
    evalPlanning(
      "SCHEDULED:   <\(date) \(day) \(time) +2w>",  // with extra spaces before timestamp
      keyword: "SCHEDULED",
      timestamp: Timestamp(active: true, date: theDate, repeater: "+2w"))
    evalPlanning(
      "   SCHEDULED: <\(date) \(day) \(time) +2w>",  // with leading space
      keyword: "SCHEDULED",
      timestamp: Timestamp(active: true, date: theDate, repeater: "+2w"))
    evalPlanning(
      "SCHEDULED: <\(date) \(day) \(time) +2w>     ",  // with trailing space
      keyword: "SCHEDULED",
      timestamp: Timestamp(active: true, date: theDate, repeater: "+2w"))
    evalPlanning(
      "    SCHEDULED: <\(date) \(day) \(time) +2w>     ",  // with leading & trailing space
      keyword: "SCHEDULED",
      timestamp: Timestamp(active: true, date: theDate, repeater: "+2w"))

    // illegal ones are considered normal line
    evalLine(
      "closed: <\(date) \(day) \(time)>",  // case sensitive
      text: "closed: <\(date) \(day) \(time)>")

    evalLine(
      "OPEN: <\(date) \(day) \(time)>",  // illegal keyword
  ...
  func testTokenListItem() {
    ...
    evalListItem(
      "1. ordered list item", indent: 0, text: "ordered list item",
      ordered: true)
    evalListItem(
      "  200) ordered list item", indent: 2, text: "ordered list item",
      ordered: true)
    // checkboxes
    evalListItem(
      "- [ ] checkbox", indent: 0, text: "checkbox", ordered: false,
      checked: false)
    evalListItem(
      "- [-] checkbox", indent: 0, text: "checkbox", ordered: false,
      checked: false)
    evalListItem(
      "- [X] checkbox", indent: 0, text: "checkbox", ordered: false,
      checked: true)
    // illegal checkboxes
    evalListItem(
      "- [] checkbox", indent: 0, text: "[] checkbox", ordered: false,
      checked: nil)
    evalListItem(
      "- [X]checkbox", indent: 0, text: "[X]checkbox", ordered: false,
      checked: nil)
    evalListItem(
      "- [Y] checkbox", indent: 0, text: "[Y] checkbox", ordered: false,
      checked: nil)
  ...
  func testTable() {
    // valid table rows
    evalTableRow(
      "| hello | world | y'all |", cells: ["hello", "world", "y'all"])
    evalTableRow(
      "   | hello | world | y'all |", cells: ["hello", "world", "y'all"])
    evalTableRow(
      "|     hello | world       |y'all |", cells: ["hello", "world", "y'all"])
