...
public struct ListItem: Node {
  ...
  public init(
    text t: String? = nil, checked c: Bool? = nil, list l: List? = nil
  ) {
    text = t
    subList = l
    checked = c
  ...
  public var description: String {
    return
      "ListItem(text: \(String(describing: text)), checked: \(String(describing: checked)), subList: \(String(describing: subList)))"
  }
}
...
extension OrgParser {
  func parseList() throws -> List {
    guard
      case let (_, Token.listItem(indent, text, ordered, checked)) =
        tokens.dequeue()!
    else {
