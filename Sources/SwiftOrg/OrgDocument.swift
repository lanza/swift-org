...
public struct OrgDocument: Node {
  ...
  private func findSectionRecursive(in nodes: [Node], targetIndex: OrgIndex) -> Section? {
    for node in nodes {
      if let section = node as? Section, section.index == targetIndex {
        return section
      }
      if let section = node as? Section {
        if let found = findSectionRecursive(in: section.content, targetIndex: targetIndex) {
          return found
        }
      }
    }
  ...
  public mutating func moveContent(from sourceIndex: OrgIndex, to targetIndex: OrgIndex, contentIndex: Int) -> Bool {
    guard var sourceSection = findSection(at: sourceIndex),
          var targetSection = findSection(at: targetIndex),
          contentIndex < sourceSection.content.count else {
      return false
    }
  ...
  public mutating func moveSubsection(from sourceTopLevelIndex: Int, subsectionIndex: Int, to targetTopLevelIndex: Int) -> Bool {
  ...
  public func subsections(in topLevelIndex: Int) -> [Section] {
    guard topLevelIndex < content.count,
          let section = content[topLevelIndex] as? Section else {
      return []
    }
  ...
  public mutating func moveSubsection(withTitle title: String, from sourceTopLevelIndex: Int, to targetTopLevelIndex: Int) -> Bool {
    guard sourceTopLevelIndex < content.count,
          targetTopLevelIndex < content.count,
          var sourceSection = content[sourceTopLevelIndex] as? Section,
          var targetSection = content[targetTopLevelIndex] as? Section else {
      return false
    }
    
    // Find the subsection with the given title
    guard let subsectionIndex = sourceSection.content.firstIndex(where: { node in
      if let section = node as? Section, section.stars == 2, section.title == title {
        return true
      }
      return false
    }) else {
    ...
    return moveSubsection(from: sourceTopLevelIndex, subsectionIndex: subsectionIndex, to: targetTopLevelIndex)
  }
  ...
  private mutating func updateSectionRecursive(in nodes: inout [Node], section: Section) {
    for i in 0..<nodes.count {
      if let existingSection = nodes[i] as? Section, existingSection.index == section.index {
        nodes[i] = section
        return
      }
