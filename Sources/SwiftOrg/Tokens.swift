...
struct TokenDescriptor {
  ...
  init(
    ...
    generator theGenerator: @escaping TokenGenerator = { matches in
      .raw(matches[0]!)
    }
  ) {
    pattern = thePattern
...
nonisolated(unsafe) var tokenDescriptors: [TokenDescriptor] = []
...
func defineTokens() {
  ...
  define("^\\s*(\(PlanningKeyword.all.joined(separator: "|"))):\\s+(.+)$") {
    matches in
    let timestamp = Timestamp.from(string: matches[2]!)
    return .planning(
      keyword: PlanningKeyword(rawValue: matches[1]!)!, timestamp: timestamp)
  }
  // Block
  define(
    "^(\\s*)#\\+begin_([a-z]+)(?:\\s+(.*))?$",
  ...
  define("^(\\s*)([-+*]|\\d+(?:\\.|\\)))\\s+(?:\\[([ X-])\\]\\s+)?(.*)$") {
    ...
    return .listItem(
      indent: length(matches[1]), text: matches[4], ordered: ordered,
      checked: checked)
  }
